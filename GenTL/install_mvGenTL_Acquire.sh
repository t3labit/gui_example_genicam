...
# Define the users real name if possible, to prevent accidental root ownership if script is invoked with sudo
...
{
    printf "Could not install the selected package(s) >%s<. You may need to install it manually" "$1"
...
if grep -q '/etc/ld.so.conf.d/' /etc/ld.so.conf; then
    ...
    $SUDO chmod a+r $GENICAM_LDSOCONF_FILE # in case umask has changed the file permissions
    $SUDO rm -f $ACQUIRE_LDSOCONF_FILE
    $SUDO touch $ACQUIRE_LDSOCONF_FILE
    $SUDO chmod a+r $ACQUIRE_LDSOCONF_FILE # in case umask has changed the file permissions
fi
...
if grep -q '/etc/profile.d/' /etc/profile; then
    ...
    $SUDO chmod a+r $GENICAM_EXPORT_FILE # in case umask has changed the file permissions
    $SUDO rm -f $ACQUIRE_EXPORT_FILE
    $SUDO touch $ACQUIRE_EXPORT_FILE
    $SUDO chmod a+r $ACQUIRE_EXPORT_FILE # in case umask has changed the file permissions
fi
...
if ! [ -r $GENICAM_EXPORT_FILE ]; then
   ...
   if ! \$(echo \$GENICAM_GENTL'$TARGET_POINTER_LENGTH'_PATH | grep -q \":$DEF_DIRECTORY/lib/$TARGET_UNCAPITALIZED\"); then
    ...
    # Since mvIMPACT Acquire version 2.28.0, version 3.1.0 of the GenICam cache should be able to coexist with
    # version 2.3, 2.4 and 3.0 however they must point to different folders!
    # Since mvIMPACT Acquire version 2.45.0, version 3.3.0 of the GenICam cache should be able to coexist with
    # version 2.3, 2.4, 3.0 and 3.1 however they must point to different folders!
    # Since mvIMPACT Acquire version 2.50.0, version 3.4.0 of the GenICam cache should be able to coexist with
    # version 2.3, 2.4, 3.0, 3.1 and 3.3.0 however they must point to different folders!
    ...
    if grep -q 'GENICAM_CACHE_V3_4=' $GENICAM_EXPORT_FILE; then
        echo 'GENICAM_CACHE_V3_4 already defined in' $GENICAM_EXPORT_FILE.
    else
        $SUDO mkdir -p $DEF_DIRECTORY/runtime/cache/v$GENICAM_VERSION
        $SUDO chmod -R 777 $DEF_DIRECTORY/runtime/cache
        $SUDO sh -c "echo 'export GENICAM_CACHE_V3_4='$DEF_DIRECTORY'/runtime/cache/v3_4' >> $GENICAM_EXPORT_FILE"
    fi
...
if [ "$MINIMAL_INSTALLATION" == "NO" ]; then
    if ([ "$REPOSITORY_ACCESS_AVAILABLE" == "YES" ] || [ "$FORCE_REPOSITORY_USAGE" == "YES" ]) || [ "$USE_DEFAULTS" == "YES" ]; then
        ...
        function showInputRequest()
        {
            INPUT_REQUEST="Do you want to install >${1}< (default is 'yes')?"
            echo "${INPUT_REQUEST}"
            echo "Hit 'n' + <Enter> for 'no', or just <Enter> for 'yes'."
        }
        ...
        if [ "$(wx-config --release 2>&1 | grep -c "^3.")" != "1" ] ||
            [ "$(wx-config --libs webview 2>&1 | grep -c "webview")" != "1" ] ||
            [ "$(wx-config --selected-config 2>&1 | grep -c "gtk3")" == "0" ]; then
            echo
            ADDITIONAL_PACKAGE_TO_BE_INSTALLED="wxWidgets"
            showInputRequest $ADDITIONAL_PACKAGE_TO_BE_INSTALLED
            echo "This is highly recommended, as without $ADDITIONAL_PACKAGE_TO_BE_INSTALLED, you cannot build wxPropView."
            echo
            if [ "$USE_DEFAULTS" == "NO" ]; then
                read YES_NO
            else
            ...
            if [ "$YES_NO" == "n" ] || [ "$YES_NO" == "N" ]; then
                ...
                if [ "$OS_NAME" == "Ubuntu" ] || [ "$OS_NAME" == "Debian" ] || [ "$OS_NAME" == "Linuxmint" ]; then
                    REQUIRED_PACKAGES=(libwxgtk-webview3.2-* libwxgtk3.2-dev build-essential libgtk2.0-dev)
                    installPackages "${REQUIRED_PACKAGES[@]}"
                    if [ $? -ne 0 ]; then
                        echo
                        echo "Could not install $ADDITIONAL_PACKAGE_TO_BE_INSTALLED v3.2, falling back to v3.0."
                        REQUIRED_PACKAGES=(libwxgtk-webview3.0-* libwxgtk3.0-dev* wx3.0-headers build-essential libgtk2.0-dev)
                        installPackages "${REQUIRED_PACKAGES[@]}"
                    fi
                    echo
...
if [ -d /etc/security/limits.d ]; then
    $SUDO touch /etc/security/limits.d/acquire.conf
    $SUDO chmod a+r /etc/security/limits.d/acquire.conf
    if [ "$(grep -c '@plugdev            -       nice            -20' /etc/security/limits.d/acquire.conf)" == "0" ]; then
...
if [ "$ERROR_NUMBER" == 0 ] && [ "$WARNING_NUMBER" == 0 ]; then
    ...
    echo "  support department (mailto:servicedesk@matrix-vision.com) if the error messages shown during   "
    echo "  the installation procedure don't help you to get the driver package installed    "
    echo "  correctly!                                                                       "
    echo "-----------------------------------------------------------------------------------"
fi
echo
